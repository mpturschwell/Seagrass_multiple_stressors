...
biomass.int.s <- function(tmax, dt, B_init, params){
  ...
  for (time_step in 1:(tmax-1)){
    B[time_step+1] <- B[time_step] + 
      dBdt.s(1, B[time_step], params)[[1]]*dt
  }
  return(B)
}

# create wrapper to list parameters before calling pmap 
biomass.int.wrapper <- function(tmax, dt, B_init, T., I., PT.max, PL.max, T.opt, T.max, Ik, B.max, 
                                R.max, RT.opt, RT.max, M){
  params <- list(
      B.max = B.max, 
      T. = T.,     
      I. = I.,     
  ...
  biomass.int.s(tmax, dt, B_init, params)
