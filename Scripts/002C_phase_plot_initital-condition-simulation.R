...
dBdt.s.consumer.foodweb_phase <- function(t, y, parameters){
...
png(filename = "Plots/phase_plot_for_IC_plot.png", width = 22, height = 11, units = "cm",  res = 300)
par(mfrow = c(1,2))
...
for (i in 1:nrow(templight)){
  ...
  { # Generate the phase plot for the given parameters
    lotkaVolterra.flowField <- flowField(dBdt.s.consumer.foodweb_phase, 
    ...
    lotkaVolterra.nullclines <- nullclines(dBdt.s.consumer.foodweb_phase, 
